...
static void contractor_cleanup(void *arg)
{
	...
	/* XXX Does it really make sense to have these callbacks? They don't happen when notify_handle_finished... */
	/* XXX What about here launching thread that pthread_joins, and then notifies handle finished? Make that thread detached. */
